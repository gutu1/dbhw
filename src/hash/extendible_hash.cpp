...
size_t ExtendibleHash<K, V>::HashKey(const K &key) {
	return std::hash<K>{}(key);//返回hash键值对应的bucket
}
 ...
 */
template <typename K, typename V>
bool ExtendibleHash<K, V>::Find(const K &key, V &value) {//找到键值对应的原文，赋值给value，若没有则返回false
	std::lock_guard<std::mutex> guard(mutex);
	auto idx = HashKey(key) & ((1 << globalDepth) - 1);
	std::shared_ptr<Bucket> bucket = bucketTable[idx];
 ...
 */
template <typename K, typename V>
bool ExtendibleHash<K, V>::Remove(const K &key) {//删除key对应的记录
	std::lock_guard<std::mutex> guard(mutex);
...
void ExtendibleHash<K, V>::Insert(const K &key, const V &value) {
	...
	auto idx = HashKey(key) & ((1 << globalDepth) - 1);
	std::shared_ptr<Bucket> targetBucket = bucketTable[idx];
	while (targetBucket->items.size() == bucketMaxSize) {//如果该bucket已满，则需要分裂
	...
	//遍历targetBucket,把它分别分到两个BucketTable
